...
let anni = {
  ...
  queryByUserId: 'SELECT a.*, u.name AS userName FROM annis a, users u WHERE a.userId=? AND a.userId=u.id',
  queryByLikerId: 'SELECT a.*, u.name AS userName, l.createAt AS likeAt FROM annis a, users u, anni_likes l WHERE l.userId=? AND l.anniId=a.id AND a.userId=u.id',
  queryByBookmarkerId: 'SELECT a.*, u.name AS userName, b.createAt AS bookmarkAt FROM annis a, users u, anni_bookmarks b WHERE b.userId=? AND b.anniId=a.id AND a.userId=u.id',
  queryByMarkerId: 'SELECT a.*, u.name AS userName, m.createAt AS markAt FROM annis a, users u, anni_marks m WHERE m.userId=? AND m.anniId=a.id AND a.userId=u.id',
